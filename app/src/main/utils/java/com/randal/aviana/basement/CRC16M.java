...
public class CRC16M {
    ...
    public static byte[] getSendBuf(byte[] toSend) {
        ...
        crc16.update(toSend, toSend.length - 2);
        int val = crc16.getValue();
        toSend[toSend.length - 1] = (byte) (0xff & val);
        toSend[toSend.length - 2] = (byte) ((0xff00 & val) >> 8);
        return toSend;
    }

    public static boolean checkBuf(byte[] bs) {
        CRC16M crc16 = new CRC16M();
        crc16.update(bs, bs.length - 2);
        int ri = crc16.getValue();
        return bs[bs.length - 1] == (byte) (ri & 0xff)
                && bs[bs.length - 2] == (byte) ((0xff00 & ri) >> 8);
    }
}
